...
# from sklearn.utils.vasklearn.dation import NotFittedError, check_non_negative
...
def _update_doc_distribution(X, exp_topic_word_distr, doc_topic_prior,
    ...
    for idx_d in xrange(n_samples):
        ...
        # doc_topic_d has size K, i.e. no. topics
        doc_topic_d = doc_topic_distr[idx_d, :]
        # The next one is a copy, since the inner loop overwrites it.
        exp_doc_topic_d = exp_doc_topic[idx_d, :].copy()
        #exp_topic_word_d has size K x W, i.e. no. topics x no. words
        exp_topic_word_d = exp_topic_word_distr[:, ids]

        # Iterate between `doc_topic_d` and `norm_phi` until convergence
        for _ in xrange(0, max_iters):
            last_d = doc_topic_d # 1 x K 
